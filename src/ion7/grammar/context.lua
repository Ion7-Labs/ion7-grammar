...
--- Mark the context as dirty so the next call to current() triggers a rebuild.
--- @private
...
function GrammarContext:learn_enum(rule_name, values)
    assert(type(rule_name) == "string" and type(values) == "table",
        "[ion7.grammar.context] learn_enum: rule_name (string) and values (table) required")
    self._enums[rule_name] = values
    return self:_invalidate()
end

--- Register a database table with its column names.
--- Creates two rules: `<table>` (the table name) and `<table>_col` (column names).
--- Also updates or creates a combined `table_name` and `column_name` enum.
---
...
function GrammarContext:learn_table(name, columns)
    assert(type(name) == "string" and type(columns) == "table",
        "[ion7.grammar.context] learn_table: name (string) and columns (table) required")
...
function GrammarContext:learn_tool(name, schema)
    assert(type(name) == "string",
        "[ion7.grammar.context] learn_tool: name must be a string")
    -- Replace if exists
...
function GrammarContext:current()
    ...
    -- Combined table_name and column_name enums (if any tables learned)
    ...
    if #self._tools > 0 then
        ...
        -- If no root yet, use tool_call as root
    ...
    -- If no explicit root rule, create one pointing to the first non-ws rule
    if not b._names[self._root] then
        ...
        if first then
            ...
            -- Empty context: root matches empty string
            b:rule(self._root, ast.literal(""))
        end
    end

    -- Wrap in Grammar_obj
    local Grammar_obj_mt = {}
    Grammar_obj_mt.__index = Grammar_obj_mt
    Grammar_obj_mt.to_gbnf = function(self_g, root)
        if root then b:root(root) end
        return b:compile()
    end
    Grammar_obj_mt.builder = function() return b end
    Grammar_obj_mt.rules   = function() return b:names() end
    Grammar_obj_mt.merge   = function() error("context grammar: use learn_* to add rules") end
    Grammar_obj_mt._builder = b

    self._cached = setmetatable({}, Grammar_obj_mt)
    self._dirty  = false
    return self._cached
end
...
function GrammarContext:snapshot()
    -- Deep copy of all learned state
    local snap = {
        root   = self._root,
        enums  = {},
        tables = {},
...
--- @return table  { n_enums, n_tables, n_tools, n_extra, rule_names }
